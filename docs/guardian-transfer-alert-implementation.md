...
대상 기능명세:
`07-01`, `07-02-03`, `07-04-07`, `10-01`, `10-03`
...
| 항목 | 설정 키 | 현재 기본값 | 비고 |
|---|---|---|---|
| 초대 유효 시간 | `movi.guardian.invitation-expire-hours` | 24 | 명세 예시값 |
...
1. **보호자 전화번호 소유 검증** — 현재는 `초대 토큰 + 로그인 계정`까지만 확인한다.
   SMS를 받은 번호의 실제 소유자인지는 검증하지 않는다.
   운영 전환 전에 SMS OTP 또는 카카오 인증 전화번호 대조를 추가해야 한다.
   (`07-02-03` Specification Gap — 임의로 OTP를 넣지 않았다.)
