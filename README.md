...
음성 WebSocket은 발화와 STT 최종 확정을 기다릴 수 있도록 프록시 읽기·쓰기 타임아웃을
600초로 둔다. 접근 로그는 인증 토큰이 포함된 query string을 남기지 않고 `$uri`만 기록한다.
...
| 음성 세션 | ✅ | 업로드 검증, 슬롯 저장·병합, 확인·취소, 만료·재시도 제한, **빠진 값을 짚어 주는 재질문**(은행·계좌번호·미등록 이름·동명이인) |
...
- 등록된 이름과 직접 말한 계좌번호 **두 경로** 모두 지원 (아래 「음성 이체 되물음과 확인」)
...
수취인을 정하는 길이 둘입니다 — **등록해 둔 이름**을 부르거나, **은행과 계좌번호**를 말하거나.
둘 중 하나만 있으면 됩니다. 무엇이 빠졌든 되묻는 문장은 그 빠진 것을 가리킵니다.

| 발화에서 받은 것 | 되묻는 말 |
|---|---|
| 아무것도 없음 | 누구에게 보내시겠어요? |
| 계좌번호만 (은행 없음) | 어느 은행 계좌인가요? 은행 이름을 말씀해 주세요. |
| 은행만 (계좌번호 없음) | 계좌번호를 말씀해 주세요. |
| 등록되지 않은 이름 | *OOO* 님은 저장돼 있지 않아요. 받는 분의 은행과 계좌번호를 말씀해 주세요. |
| 비슷한 이름이 여럿 | *OOO* 님과 비슷한 이름이 여러 개 저장돼 있어요. 받는 분의 계좌번호를 말씀해 주세요. |
| 금액 없음 | 얼마를 보내시겠어요? |

- **문구는 상황마다 하나로 고정합니다.** 같은 상황에서 매번 다른 말이 나오면 화면을 보지 않는 사용자는 무엇이 바뀐 것인지 알 수 없습니다.
- 이름을 못 찾는 것은 **오류가 아니라 되물을 일**입니다. 예외로 끊으면 "저장된 분이 없어요"에서 대화가 끝나, 계좌번호를 말하면 보낼 수 있다는 것을 사용자가 알 수 없습니다.
- 비슷한 이름이 여럿이면 **임의로 고르지 않습니다.** 편집 거리 1 이하 후보가 정확히 하나일 때만 보정하고, 둘이면 계좌번호로 가려 달라고 되묻습니다. 이때 "저장된 분이 없어요"라고 하지 않습니다 — 등록은 돼 있고 고르지 못한 것이라 사실과 다릅니다.
- 되물어도 **절반만 들은 계좌 정보는 지킵니다.** 은행만 듣고 지우면 사용자가 계좌번호를 답해도 은행이 없어 또 되묻게 됩니다. 다만 은행도 계좌번호도 못 들었을 때는 버립니다 — 낡은 계좌번호가 남아 있으면 사용자가 이름으로 답해도 계좌 쪽이 우선해 엉뚱한 곳으로 나갑니다.
- `missingSlots` 로 내보내는 값은 `RECIPIENT`·`AMOUNT` 둘뿐입니다. 프런트가 이 둘만 허용하도록 응답을 검증하므로, 슬롯을 늘리려면 프런트와 함께 바꿔야 합니다. 무엇이 빠졌는지는 슬롯이 아니라 `voiceMessage` 가 전달합니다.

확인 복창은 수취인을 어떻게 정했는지에 따라 다릅니다.

```text
등록된 이름     생활비 통장에서 엄마 님에게 오만원을 보낼까요?
계좌번호 직접   생활비 통장에서 국민은행 7666 계좌 삼 오 이 공 ... 으로 오만원을 보낼까요?
```

계좌번호를 말해서 보낼 때는 **뒤 네 자리가 아니라 전체를 한 자리씩** 읽습니다. 등록해 둔 상대가
아니라 방금 부른 번호로 나가므로, 가운데를 잘못 들은 것을 잡을 수단이 이 복창뿐입니다.
등록된 이름으로 보낼 때는 사용자가 부른 이름을 그대로 되읽는 편이 알아듣기 쉬워 번호를 읽지
않습니다.

두 경우 모두 `맞으면 "네 맞아요", 아니면 "아니요 취소할게요"` 안내를 붙입니다. `"네"` 한 음절은
STT 가 잡지 못해 — 운영 기록상 확인에 성공한 발화는 전부 두 어절 이상이었습니다.
