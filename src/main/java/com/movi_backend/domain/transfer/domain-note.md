...
- **2026-09-04** — 계좌번호로 보낼 때 사용자가 부른 이름을 남긴다 (#126). 이름을 말해도 버리고 "국민은행 6789"로 저장하고 있었다. 수취인 목록은 화면을 볼 수 없는 사용자가 계좌번호를 외우지 않고 송금할 유일한 수단인데, 그 이름으로는 다음에 부를 수 없고 같은 은행으로 몇 건 쌓이면 구분도 되지 않는다. **수취인은 지우거나 이름을 바꿀 수단이 없어 그 상태가 그대로 남는다.**
  - 이름을 말했고 신뢰도가 충분하면 그 이름을 별칭으로 쓴다. 아니면 지금까지처럼 은행·뒤 네 자리로 만든다
  - `holderName`은 그대로 은행·뒤 네 자리다. 실명을 확인할 방법이 없어, 사용자가 그렇게 불렀다는 사실이 계좌 주인이 그 사람이라는 근거가 되지 않는다
  - 확인 복창은 계좌번호 전체를 계속 읽는다. 이름이 붙었다고 자릿수 확인을 건너뛰면 잘못 들은 번호를 잡을 수단이 사라진다
  - 별칭 중복 처리(`uniqueNickname`)를 사람 이름과 자동 생성 이름이 함께 쓴다
