...
| AUTH_4015 | `INVALID_LOGIN_CODE` | 401 | 로그인 교환 코드가 유효하지 않습니다. | 로그인을 처음부터 다시 시도해 주세요. |
...
| AUTH_4024 | `PASSWORD_MISMATCH` | 401 | 아이디 또는 비밀번호가 일치하지 않습니다. | 아이디나 비밀번호가 맞지 않아요. 다시 입력해 주세요. |
| AUTH_4025 | `PASSWORD_LOCKED` | 403 | 로그인 실패 제한 횟수를 초과했습니다. | 여러 번 잘못 입력하셨어요. 오 분 뒤에 다시 시도해 주세요. |
| AUTH_4026 | `PASSWORD_NOT_REGISTERED` | 400 | 일반 로그인 비밀번호가 등록되어 있지 않습니다. | 이 계정은 카카오로 로그인해 주세요. |
| AUTH_4092 | `LOGIN_ID_ALREADY_REGISTERED` | 409 | 이미 사용 중인 아이디입니다. | 이미 쓰고 있는 아이디예요. 다른 아이디를 정해 주세요. |
...
| ACCOUNT_4005 | `ACCOUNT_HAS_PENDING_TRANSFER` | 400 | 진행 중인 이체가 있어 계좌 연결을 해제할 수 없습니다. | 보내는 중인 돈이 있어요. 끝난 뒤에 다시 해제해 주세요. |
| ACCOUNT_4006 | `ACCOUNT_SELECTION_REQUIRED` | 400 | 조회할 계좌를 특정할 수 없습니다. | 같은 은행 계좌가 여러 개예요. 계좌 별칭을 말씀해 주세요. |
...
| VOICE_4012 | `CONFIRMATION_KEY_MISSING` | 400 | 확인 발화에 멱등성 키가 없습니다. | 송금 확인을 처리하지 못했어요. 마이크를 눌러 다시 한 번 답해 주세요. |
