-- =====================================================
-- Movi MVP — ERDCloud 임포트용 DDL (테이블명 한글+영어 병기)
-- 사용법: ERDCloud → [Import] → [DDL] → 전체 붙여넣기
-- 주의: ERD 시각화 전용. 실제 DB 생성은 docs/schema.sql 사용
-- =====================================================
...
CREATE TABLE `소셜로그인_oauth_accounts` (
    ...
    CONSTRAINT fk_oauth_user FOREIGN KEY (user_id) REFERENCES `회원_users` (user_id)
) COMMENT '소셜 로그인 계정';
...
CREATE TABLE `등록기기_devices` (
    ...
    CONSTRAINT fk_device_user FOREIGN KEY (user_id) REFERENCES `회원_users` (user_id)
) COMMENT '등록 기기';

CREATE TABLE `접근성설정_accessibility_settings` (
    ...
    CONSTRAINT fk_setting_user FOREIGN KEY (user_id) REFERENCES `회원_users` (user_id)
) COMMENT '접근성 설정';

CREATE TABLE `오픈뱅킹연결_openbanking_connections` (
    ...
    CONSTRAINT fk_conn_user FOREIGN KEY (user_id) REFERENCES `회원_users` (user_id)
) COMMENT '오픈뱅킹 연결';

CREATE TABLE `계좌_accounts` (
    ...
    CONSTRAINT fk_account_user FOREIGN KEY (user_id) REFERENCES `회원_users` (user_id),
    CONSTRAINT fk_account_conn FOREIGN KEY (connection_id) REFERENCES `오픈뱅킹연결_openbanking_connections` (connection_id)
) COMMENT '연결 계좌';

CREATE TABLE `잔액스냅샷_balance_snapshots` (
    ...
    CONSTRAINT fk_snapshot_account FOREIGN KEY (account_id) REFERENCES `계좌_accounts` (account_id)
) COMMENT '잔액 스냅샷';

CREATE TABLE `음성세션_voice_sessions` (
    ...
    pending_intent VARCHAR(40) NULL COMMENT '대기 중인 의도',
    ...
    CONSTRAINT fk_vsession_user FOREIGN KEY (user_id) REFERENCES `회원_users` (user_id),
    CONSTRAINT fk_vsession_device FOREIGN KEY (device_id) REFERENCES `등록기기_devices` (device_id)
) COMMENT '음성 세션';

CREATE TABLE `음성명령_voice_commands` (
    ...
    CONSTRAINT fk_vcmd_session FOREIGN KEY (session_id) REFERENCES `음성세션_voice_sessions` (session_id),
    CONSTRAINT fk_vcmd_user FOREIGN KEY (user_id) REFERENCES `회원_users` (user_id)
) COMMENT '음성 명령';

CREATE TABLE `거래내역_transactions` (
    ...
    CONSTRAINT fk_tran_account FOREIGN KEY (account_id) REFERENCES `계좌_accounts` (account_id)
) COMMENT '거래 내역';

CREATE TABLE `수취인_transfer_recipients` (
    ...
    CONSTRAINT fk_recipient_user FOREIGN KEY (user_id) REFERENCES `회원_users` (user_id)
) COMMENT '자주 쓰는 수취인';

CREATE TABLE `이체_transfers` (
    ...
    CONSTRAINT fk_transfer_user FOREIGN KEY (user_id) REFERENCES `회원_users` (user_id),
    CONSTRAINT fk_transfer_account FOREIGN KEY (from_account_id) REFERENCES `계좌_accounts` (account_id),
    CONSTRAINT fk_transfer_recipient FOREIGN KEY (recipient_id) REFERENCES `수취인_transfer_recipients` (recipient_id),
    CONSTRAINT fk_transfer_vcmd FOREIGN KEY (voice_command_id) REFERENCES `음성명령_voice_commands` (command_id)
) COMMENT '이체 요청';

CREATE TABLE `FDS위험평가_fds_assessments` (
    ...
    CONSTRAINT fk_fds_transfer FOREIGN KEY (transfer_id) REFERENCES `이체_transfers` (transfer_id),
    CONSTRAINT fk_fds_user FOREIGN KEY (user_id) REFERENCES `회원_users` (user_id)
) COMMENT 'FDS 위험 평가';

CREATE TABLE `FDS룰_fds_rules` (
...
CREATE TABLE `FDS평가룰매핑_fds_assessment_rules` (
    ...
    PRIMARY KEY (id),
    CONSTRAINT fk_far_assessment FOREIGN KEY (assessment_id) REFERENCES `FDS위험평가_fds_assessments` (assessment_id),
    CONSTRAINT fk_far_rule FOREIGN KEY (rule_id) REFERENCES `FDS룰_fds_rules` (rule_id)
) COMMENT 'FDS 평가-룰 매핑';

CREATE TABLE `이체행동프로필_user_transfer_profiles` (
    user_id                 BIGINT        NOT NULL COMMENT '사용자 ID',
    ...
    updated_at              DATETIME      NOT NULL COMMENT '수정일시',
    PRIMARY KEY (user_id),
    CONSTRAINT fk_profile_user FOREIGN KEY (user_id) REFERENCES `회원_users` (user_id)
) COMMENT '사용자 이체 행동 프로필';

CREATE TABLE `보호자연결_guardian_links` (
    ...
    CONSTRAINT fk_glink_protectee FOREIGN KEY (protectee_user_id) REFERENCES `회원_users` (user_id),
    CONSTRAINT fk_glink_guardian FOREIGN KEY (guardian_user_id) REFERENCES `회원_users` (user_id)
) COMMENT '보호자 연결';

CREATE TABLE `알림_notifications` (
    ...
    CONSTRAINT fk_noti_user FOREIGN KEY (user_id) REFERENCES `회원_users` (user_id),
    CONSTRAINT fk_noti_link FOREIGN KEY (link_id) REFERENCES `보호자연결_guardian_links` (link_id),
    CONSTRAINT fk_noti_transfer FOREIGN KEY (transfer_id) REFERENCES `이체_transfers` (transfer_id)
) COMMENT '알림 발송';

CREATE TABLE `감사로그_audit_logs` (
    ...
    created_at    DATETIME     NOT NULL COMMENT '생성일시',
    PRIMARY KEY (log_id),
    CONSTRAINT fk_audit_user FOREIGN KEY (user_id) REFERENCES `회원_users` (user_id)
) COMMENT '감사 로그';
