...
> 이 문서는 **도메인이 지켜야 할 설계 계약**입니다. 구현 현황은 [README](../README.md),
> 패키지 내부 관점의 보충 설명은 각 도메인의 `domain-note.md`를 보세요.
> 실제 구현과 달라지면 이 문서를 갱신하세요.
...
- **불변식**
  ...
  - 2026-09-02 실제 AI 서버(`POST /api/v1/fraud/detect`)로 확인: `risk_level`을 AI가 규칙엔진으로
    이미 LOW/MEDIUM/HIGH 판정해 준다. 백엔드는 그 값을 그대로 받아 쓴다 — 점수 구간을 백엔드가
    새로 정의하지 않는다. `rule_score`·`final_risk_score`는 AI 쪽이 0~100 스케일이라 내부
    0~1 스케일로 나누기만 한다(값의 재해석이 아니라 단위 변환). 자세한 내용은
    `domain/fds/domain-note.md` 참고
  - 타임아웃은 **3초, 자동 재시도 없음**
...
- **`*.yml`은 전부 gitignore 대상이다.** 인증정보를 파일에 직접 적기 때문이다. `.example` 템플릿은 쓰지 않으며, 필요한 값은 팀 채널(Notion/카톡)에서 공유한다
  - clone 직후에는 설정 파일이 없다. 직접 만들어야 기동된다
  - 설정 항목을 추가·변경했다면 **팀 채널에도 함께 공유**한다. 안 그러면 다른 팀원이 기동에 실패한다
...
- `transfer_recipients.account_num`, `transfers.to_account_num`에는 AES-GCM 암호문만 저장한다.
- FDS 평가와 거래 이력 저장에는 암호문을 유지하고, 실제 오픈뱅킹 이체 요청을 만들 때만
  수취 계좌번호를 복호화한다.
- **예외 메시지는 TTS로 읽힌다**
